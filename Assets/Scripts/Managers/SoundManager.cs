...
public class SoundManager : MonoBehaviour
{
    ...
    public AudioSource Play(AudioClip clip, Vector3 position, float volume = 1.0f, float pitch = 1.0f, bool loop = false)
    {
        foreach (var audioSource in audioSources)
        {
            if (!audioSource.isPlaying)
            {
                Play(audioSource, clip, position, volume, pitch, loop);
                return audioSource;
            }
        }
        ...
        AudioSource newAudioSource = newAudioSourceObject.AddComponent<AudioSource>();
        newAudioSource.minDistance = minRange;
        newAudioSource.maxDistance = maxRange;
        newAudioSource.dopplerLevel = 0.0f;
        audioSources.Add(newAudioSource);

        Play(newAudioSource, clip, position, volume, pitch, loop);
    ...
    void Play(AudioSource audioSource, AudioClip clip, Vector3 position,float volume = 1.0f, float pitch = 1.0f, bool loop = false)
    {
        audioSource.clip = clip;
        audioSource.volume = volume * masterVolume;
        audioSource.pitch = pitch;
